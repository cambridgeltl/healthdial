...
def register():
    ...
    if registerdata:
        ...
        _age_group = registerdata["age_group"]
        _gender = registerdata["gender"]
        _region_of_residence = registerdata["region_of_residence"]
        _place_of_origin = registerdata["place_of_origin"]
        _primary_language = registerdata["primary_language"]
        _secondary_languages = registerdata.get("secondary_languages", "")
        _education_level = registerdata["education_level"]
        ...
        _newUser = LoginUser(username=_user_name, email=_email, password=_password, country=_country,
                             ...
                             place_of_origin=_place_of_origin,
                             primary_language=_primary_language,
                             secondary_languages=_secondary_languages,
...
def get_task():
    ...
    try:
        ...
        if _currentTask:
            ...
            return jsonify({'success': False, 'msg': 'task all done', 'task': _currentTask}), 200
...
def save_result():
    ...
    UPLOAD_FOLDER = 'final_submit'
    if not os.path.exists(UPLOAD_FOLDER):
        os.makedirs(UPLOAD_FOLDER)

    try:
        _currentUser = LoginUser().check_user_with_id(current_user_id)
        save_data = request.files.getlist('audio')
        transcription=request.form.getlist('transcription')
        asr_result = request.form.getlist('asr_result')
        ...
        if not save_data:
            return jsonify({'success': False, 'msg': 'No audio files submitted'}), 400

        task_collection = pymongo.db.task_info
        assigned_task = task_collection.find_one({
            "task_id": task_id,
        ...
        if not assigned_task:
            return jsonify({
                'success': False,
                'msg': 'This task is not assigned to the current user or has already been submitted.'
            }), 409

        asr_service = AudioAsr()
        submission_time = datetime.now()

        save_result = asr_service.final_result_submission(
            asr_result, transcription, save_data, task_id,
            current_user_id, language, _currentUser.email, start_time
        )
        ...
        if update_result.modified_count != 1:
            logging.error(f"Task {task_id} was saved but could not be marked complete for {_currentUser.email}.")
            return jsonify({'success': False, 'msg': 'Task state changed during submission'}), 409

        return jsonify({'success': True, 'msg': save_result}), 200

    except Exception as e:
        logging.error('Error while saving task data', exc_info=True)
        return jsonify({'success': False, 'msg': 'Failed to save task data'}), 400
