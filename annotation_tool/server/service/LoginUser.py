...
class LoginUser:
    ...
    def __init__(self, username=None, email=None, password=None, country=None, age_group=None, gender=None,
                 region_of_residence=None, place_of_origin=None, primary_language=None,
                 secondary_languages=None, education_level=None):
        """
        ...
        Args:
            ...
            region_of_residence (str): The region of residence of the user.
            place_of_origin (str): The place of origin of the user.
            primary_language (str): The primary language of the user.
            secondary_languages (str): Secondary languages spoken by the user.
        ...
        self.gender = gender
        self.region_of_residence = region_of_residence
        self.place_of_origin = place_of_origin
        self.primary_language = primary_language
        self.secondary_languages = secondary_languages
    ...
    def register_new_user(self):
        ...
        Unew = User(
            ...
            role=_this_user_role.model_dump()
        )
    ...
    def check_user_with_email(self, email):
        ...
        try:
            ...
            if user:
                ...
                self.age_group = user.get('age_group')
                self.gender = user.get('gender')
                self.region_of_residence = user.get('region_of_residence')
                self.place_of_origin = user.get('place_of_origin')
                self.primary_language = user.get('primary_language')
                self.secondary_languages = user.get('secondary_languages')
                self.education_level = user.get('education_level')
    ...
    def get_task(self):
        """
        Get a new task from the task environment if the user has submitted fewer than the threshold number of tasks.

        Returns:
            dict or None: The task obtained from the task environment, or None if the user has reached the submission limit.
        """
        # Ensure the user's email is set
        if not self.email:
            raise ValueError("Email is not set for the user.")

        try:
            task_threshold = config.get_task_threshold()
            task_count = pymongo.db["final_submission"].count_documents({"user_email": self.email})
            if task_count >= task_threshold:
                logging.info(
                    f"User {self.email} has reached the submission limit "
                    f"({task_threshold} tasks)."
                )
                return None

            task = self.task_env.get_task(self.email)

            if not task:
                logging.warning(f"No task available for user {self.email}.")
                return None

            return task

        except Exception as e:
            logging.error(f"Error occurred while getting a task for user {self.email}: {str(e)}", exc_info=True)
            return None



    def save_result(self, submission):
        """
        ...
        """
        insert_result = pymongo.db['result'].insert_one(submission.model_dump())
