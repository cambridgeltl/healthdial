...
const TaskCard: React.FC<TaskCardProps> = ({ style, turn_id = -1, task_id, form }) => {
    ...
    const validateData = (_: any, value: any) => {
        ...
        if (current === 0 || current === 1) {
            return Promise.reject(new Error('Please click next before proceeding.'));
        }
    ...
    return (
        <div>
            {contextHolder}
            <Card size="small" hoverable={true} style={style}>
                <Steps current={current} items={step_items} />

                <Form.Item name={[`transcription_${turn_id}`]} rules={[{ validator: validateData }]}>
                    <Spin tip="Submitting..." spinning={submitting}>
                        <div style={contentStyle}>
                            ...
                            {current === 2 && (
                                <div>
                                    <Result icon={<SmileOutlined />} title="Great, we have completed this card!" />
                                </div>
                            )}
                        </div>
                    </Spin>
